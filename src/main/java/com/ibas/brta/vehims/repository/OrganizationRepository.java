...
public interface OrganizationRepository extends JpaRepository<Organization, Long> {
    ...
    @Query("SELECT s FROM Organization s WHERE (:nameEn IS NULL OR LOWER(s.nameEn) LIKE LOWER(CONCAT('%', :nameEn, '%')) OR LOWER(s.nameBn) LIKE LOWER(CONCAT('%', :nameEn, '%'))) AND (:officeTypeId IS NULL OR s.officeTypeId = :officeTypeId) AND (:isActive IS NULL OR s.isActive = :isActive) ORDER BY s.createdAt DESC")
