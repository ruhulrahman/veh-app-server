...
public interface StatusGroupRepository extends JpaRepository<StatusGroup, Long> {
        ...
        List<StatusGroup> findAll(Sort sort);

        void deleteByStatusGroupCode(String statusGroupCode);

        // Complex query with JPQL and named parameters
        @Query("SELECT s FROM StatusGroup s WHERE (:nameEn IS NULL OR LOWER(s.nameEn) LIKE LOWER(CONCAT('%', :nameEn, '%')) OR LOWER(s.nameBn) LIKE LOWER(CONCAT('%', :nameEn, '%')) OR LOWER(s.statusGroupCode) LIKE LOWER(CONCAT('%', :nameEn, '%'))) AND (:isActive IS NULL OR s.isActive = :isActive) ORDER BY s.nameEn ASC")
