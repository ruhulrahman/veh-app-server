...
public interface RegistrationApplicationRepository extends JpaRepository<VehicleInfo, Long> {
        ...
        @Query(value = "SELECT" +
                        ...
                        "    (case when :applicationStatusId is null then true else sr.application_status_id IS NOT NULL AND sr.application_status_id != :applicationStatusId end) AND" +
                        ...
                        +
                        "", nativeQuery = true)
        Page<RegistrationApplications> searchVehRegApplicationsForApproval(String serviceRequestNo, String chassisNumber,
                        String engineNumber, String nid, String mobile,
                        Date applicationDate, Long orgId, Long applicationStatusId, Pageable pageable);
        ...
        @Query(value = "SELECT" +
                ...
                "    LEFT JOIN c_vehicle_classes cvc on vi.vehicle_class_id=cvc.vehicle_class_id" +
                "            WHERE" +
                "    (case when :inspectorId is null then true else sr.inspector_id = :inspectorId end) AND" +
                "    (case when :orgId is null then true else sr.org_id = :orgId end) AND" +
                "    (case when :applicationStatusId is null then true else sr.application_status_id IS NOT NULL AND sr.application_status_id != :applicationStatusId end) AND" +
                "    (case when :serviceRequestNo is null or :serviceRequestNo = '' then true else sr.service_request_no = LOWER(:serviceRequestNo) end) AND"
                +
                ...
                +
                "", nativeQuery = true)
        Page<RegistrationApplications> searchVehRegApplicationsForInspector(String serviceRequestNo, String chassisNumber,
                                                                           String engineNumber, String nid, String mobile,
                                                                           Date applicationDate, Long orgId, Long applicationStatusId, Long inspectorId, Pageable pageable);
        ...
        @Query(value = "SELECT COUNT(sr.service_request_id) " +
                "FROM v_vehicle_infos vi " +
                "JOIN v_service_requests sr ON vi.vehicle_info_id = sr.vehicle_info_id " +
                "Left JOIN v_vehicle_owners vo ON vi.vehicle_info_id = vo.vehicle_info_id " +
                ...
                "AND (:vehicleTypeId IS NULL OR vi.vehicle_type_id = :vehicleTypeId) " +
                "AND (:ownerTypeId IS NULL OR vo.owner_type_id = :ownerTypeId) " +
                "AND (:applicationDate IS NULL OR DATE(sr.application_date) = CAST(:applicationDate AS DATE))",
                nativeQuery = true)
        Long getVehicleRegistrationReport(Long serviceId, Long applicationStatusId, Long orgId, Long vehicleTypeId, Long ownerTypeId, String applicationDate);
