...
public interface CommonRepository extends JpaRepository<User, Long> {
        ...
        @Query(value = "SELECT status_id FROM c_statuses WHERE status_code in (:statusCodes)", nativeQuery = true)
        List<Long> getStatusIdsByStatusCodes(@Param("statusCodes") List<String> statusCodes);
        ...
        @Query(value = "SELECT venue.office_exam_center_id as id, venue.address_en as nameEn, venue.address_bn as nameBn "
                        + "FROM c_office_jurisdictions as oj "
                        + "JOIN c_office_exam_centers as venue ON oj.org_id = venue.org_id "
                        + "WHERE oj.thana_id = :thanaId", nativeQuery = true)
